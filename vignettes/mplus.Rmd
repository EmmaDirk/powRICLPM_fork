...
::: {.callout-warning}
Note that it is not possible to generate skewed or kurtosed data in Mplus via the `powRICLPM` package. Furthermore, bounded estimation and freely estimated random-intercept factor loadings are not available in Mplus. The `ME` constraint can be combined with other Mplus-supported constraints when `estimate_ME = TRUE`.
:::
