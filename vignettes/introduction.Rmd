...
- `lagged_effects`: Standardized autoregressive and cross-lagged effects for the within-unit components of the model. These values are collected in a matrix, *with columns representing predictors and rows representing outcomes*.
- `within_cor`: A correlation for the within-unit components.
- `intraclass_correlation`: The proportion of variance at the between-unit level (relative to the total variance).
...
If you are unsure if you have specified the `lagged_effects` matrix as intended, you can use the `check_lagged_effects()` function to give you a summary of how the lagged effects are interpreted.
...
- the population values `lagged_effects`, `within_cor`, `intraclass_correlation`, and `RI_cor`, and
...
- `seed`: An integer to control the starting point of the random number generator. This is important to use if you want to replicate the results. When no seed is specified, a random seed will be generated and reported back to you.
...
Load the `future` package, save the current plan, and use `plan()` to change the power analysis execution from *sequential* (i.e., single-core, the default), to *multisession* (i.e., multicore). Next, run the `powRICLPM` analysis, and the power analysis will run across available workers. This can result in a significant reduction of computing time. For more information on other parallel execution strategies using futures, see `?future::plan()`.
...
```{r future-setup, eval = FALSE}
# Load `future` and `progressr` packages
library(future)
library(progressr)

# Restore the user's previous plan when this example exits
old_plan <- future::plan()
on.exit(future::plan(old_plan), add = TRUE)

# Plan powRICLPM analysis to run in parallel
plan(multisession)
...
`summary()` can be used in one of four ways. First, summary can be used simply like `print()` to get information about the design of the power analysis (the different experimental conditions), as well as the number of problems that occurred per condition (e.g., non-convergence, fatal estimation errors, or inadmissible results). Second, by specifying the `parameter = "..."` argument in `summary()`, the function will print the results specifically for that parameter across all experimental conditions. Third, if you specify a specific experimental condition using `summary()`'s `sample_size`, `time_points`, `intraclass_correlation` and `reliability` arguments, performance measures are outputted for all parameters in that experimental condition.
...
# Summary of all parameters for a specific simulation condition
summary(output, sample_size = 500, time_points = 4, intraclass_correlation = 0.5, reliability = 1)
...
1. `what = "conditions"` gives the different experimental conditions per row, where each condition is defined by a unique combination of sample size, number of time points, and intraclass correlation.
2. `what = "estimation_problems"` gives the number of fatal errors, inadmissible solutions, or non-converged estimations across replications for each experimental condition.
3. `what = "results"` gives the average estimate `average`, minimum estimate `minimum`, empirical standard error of parameter estimates `EmpSE`, the average standard error `SEAvg`, the mean square error `MSE`, the average width of the confidence interval `accuracy`, the coverage rate `coverage`, and the proportion of times the *p*-value was lower than the significance criterion `power`. It requires setting the `parameter = "..."` argument.
