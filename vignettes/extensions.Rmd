...
Bounded estimation can be combined with `constraints = "RI_loadings_free"`, because that option frees random-intercept loadings rather than imposing equality or time-invariance constraints. It still cannot be combined with equality or time-invariance constraints such as `constraints = "lagged"`, `constraints = "residuals"`, `constraints = "stationarity"`, or `constraints = "ME"`.
...
- `constraints = c("lagged", "residuals")`: Both lagged effects and residual variances and covariances are constrained to be equal over time.
...
- `constraints = "RI_loadings_free"`: Random-intercept factor loadings are freely estimated from the second wave onward in the estimation model. The first loading remains fixed to 1 for identification.

Compatible constraints can be combined in a character vector. For example, `constraints = c("lagged", "RI_loadings_free")` constrains the lagged effects over time and freely estimates the random-intercept factor loadings from the second wave onward. The option `constraints = "within"` is retained as shorthand for `constraints = c("lagged", "residuals")`, but the explicit vector form is the current way to specify this combination.

The `RI_loadings_free` option only changes the estimation model. Data generation remains based on the standard RI-CLPM population model, in which all random-intercept factor loadings are fixed to 1. This option is available for `software = "lavaan"` only. With `software = "Mplus"`, the vector-valued `constraints = c("lagged", "residuals")` form is accepted and treated as `constraints = "within"`. The `ME` constraint can also be combined with other constraints that are available for Mplus, for example `constraints = c("ME", "lagged")` when `estimate_ME = TRUE`.
