...
- `constraints = "RI_loadings_free"`: By default, the RI-CLPM estimation model fixes all random-intercept factor loadings to 1, so that each observed variable loads equally on its random intercept. This option relaxes that assumption in the estimation model by freely estimating the random-intercept factor loadings from the second wave onward. The first loading remains fixed to 1 for identification.

Compatible constraints can be combined in a character vector. For example, `constraints = c("lagged", "RI_loadings_free")` constrains the lagged effects over time and freely estimates the random-intercept factor loadings from the second wave onward. Similarly, `constraints = c("lagged", "residuals")` imposes the same constraints as `constraints = "within"`.

Some constraints cannot be combined because they impose overlapping or incompatible restrictions. For example, `constraints = "stationarity"` already imposes restrictions on the residual variances, so it cannot be combined with `constraints = "residuals"`. Similarly, because `constraints = "within"` already combines lagged and residual constraints, it cannot be combined with `"lagged"` or `"residuals"` in the same constraint vector.

The `RI_loadings_free` option is available for `software = "lavaan"` only. With `software = "Mplus"`, `constraints = c("lagged", "residuals")` is equivalent to `constraints = "within"`. The `ME` constraint can also be combined with other constraints that are available for Mplus, for example `constraints = c("ME", "lagged")` when `estimate_ME = TRUE`.
