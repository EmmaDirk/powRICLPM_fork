...
The `RI_loadings_free` option only changes the estimation model. Data generation remains based on the standard RI-CLPM population model, in which all random-intercept factor loadings are fixed to 1. This option is currently available for `software = "lavaan"` only. With `software = "Mplus"`, the vector-valued `constraints = c("lagged", "residuals")` form is accepted and treated as `constraints = "within"`; other vector-valued combinations are not supported.
