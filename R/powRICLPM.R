...
#'
#' \subsection{Using Mplus}{When \code{software = "Mplus"}, Mplus input files will be generated and saved into \code{save_path}. Note that it is not possible to generate skewed or kurtosed data in Mplus via the `powRICLPM` package. Furthermore, bounded estimation and freely estimated random-intercept factor loadings are not available in Mplus. Therefore, the `skewness`, `kurtosis`, and `bounds` arguments cannot be used when `software = "Mplus"`, and \code{constraints = "RI_loadings_free"} is only available when \code{software = "lavaan"}. With \code{software = "Mplus"}, \code{constraints = c("lagged", "residuals")} is equivalent to \code{constraints = "within"}. The \code{"ME"} constraint can be combined with other Mplus-supported constraints when \code{estimate_ME = TRUE}. }
#'
