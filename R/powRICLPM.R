...
#' @param loadings (optional) A \code{numeric} vector or matrix specifying random-intercept loadings in the lavaan data-generating model. A vector of length \code{time_points} is applied to both variables. A matrix must have two rows, one for each variable, and one column per time point. The first loading must be 1, so later values are interpreted relative to the first occasion. Supplying \code{loadings} requires \code{constraints = "RI_loadings_free"}. Use \code{\link{check_loadings}} to check how a loading vector or matrix is interpreted.
