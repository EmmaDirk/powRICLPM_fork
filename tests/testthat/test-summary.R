...
test_that("summary.powRICLPM asks for reliability when condition matching is ambiguous", {
  lagged_effects <- matrix(c(0.4, 0.15, 0.2, 0.3), ncol = 2, byrow = TRUE)
  out <- powRICLPM(
    target_power = 0.8,
    sample_size = 500,
  ...
  expect_error(
    summary(out, sample_size = 500, intraclass_correlation = 0.4, time_points = 3),
    "Multiple experimental conditions.*reliability"
  )
