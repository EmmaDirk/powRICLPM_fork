...
test_that("DPM constraint checks reject unsupported options", {
  ...
  expect_error(icheck_DPM_constraints(c("none", "none")), "c\\('none', 'none'\\)")
  expect_error(icheck_DPM_constraints(c("lagged", "lagged", "residuals")), "c\\('lagged', 'lagged', 'residuals'\\)")
  expect_error(icheck_DPM_constraints(matrix("AF_loadings_free", nrow = 1)), "matrix or array")
  expect_error(icheck_DPM_constraints(array(c("lagged", "AF_loadings_free"), dim = c(1, 2, 1))), "matrix or array")
