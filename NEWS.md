...
* Added a lavaan-only `loadings` argument to `powRICLPM()` for specifying random-intercept loadings in the data-generating model. The first occasion must be fixed to 1, and later loadings can vary over time and across variables.
